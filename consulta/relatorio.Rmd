...
Um modelo multinível, sem variáveis de segundo nível, pode ser descrito da seguinte forma:

$$ logit (\pi_{ij}) = \beta_{0j} + \sum_{k}{ \beta_{k}*X_{kij}} + \epsilon_{ij} $$  
com $$ \beta_{0j} = \beta_{0} + u_{0j}  $$

para $j = 1,...,n$ clusters de cursos, com $i = 1,..., n_{j}$ alunos no cluster $j$, e $k = 1,...,m$ variáveis independentes no modelo.
...
Os $X_{kij}$ são as variáveis independentes utilizadas no modelo, os 
coeficientes $\beta_k$ são os efeitos fixos do modelo. Os erros $\epsilon_{ij}$ têm
...
com $$ \beta_{0j} = \beta_{00} + \beta_{02}*Z_j + u_{0j}$$
    $$ \beta_{kj} = \beta_{k0} + \beta_{k2}*Z_j$$
...
coeficientes $\beta_{kj}$ são os efeitos fixos do modelo. Já $Z_j$ é uma variável no nível de cluster (nível 2)
$ \beta_{k2}$ são os coeficientes de regressão associados à variável explicativa do nível 2 relativo à inclinação. É importante destacar que componentes aleatórios podem ser acrescentados aos coeficientes das variáveis independentes ($\beta_{kj}$)
...
multinível. Destacamos que, de acordo com a literatura, o nível de significância estátisca selecionado para testar as variável será de: p-valor < 0.05
...
observações pois é uma representação gráfica que ilustra o desempenho de um modelo classificador binário . 
  
Se o resultado de uma previsão é 1 e o valor real também é 1, então o resultado é chamado de verdadeiro possitivo; no entanto, se o valor real é 0 e o valor previsto foi 1 então dizemos que o resultado é um falso positivo. Por outro lado, um verdadeiro negativo  ocorre quando o resultado da previsão é 0 e o valor real também é 0, e um falso negativo é quando o resultado da previsão é 0 enquanto o valor real é 1
...
verdadeiros positivos. O melhor método de previsão possível produziria um ponto no canto superior esquerdo do plano descrito pelo espaço, isto é, o ponto com coordenadas (0,1). Nesse ponto temos 100% de sensibilidade (isto é, não temos falsos negativos) e 100% de especificidade (isto é, não temos falsos positivos). O ponto (0,1) também é denominado "classificação perfeita"
...
ajuste. Como P < 0.001, há indícios para rejeitar a hipótese nula e utilizar,
...
Percebe-se que agora na estrutura de variância do modelo nós temos a covariância entre os dois efeitos aleatórios, ou seja, $\sigma(u_{0j},u_{1j})$. Nesse caso, temos duas opções de estrutura:
  
1) Uma estrutura de covariância que permite variâncias distinta para cada
efeito dentro de uma equação de efeitos aleatórios (var($u_{0j}$) $\neq$ var($u_{1j}$)) e assume que todas as covariâncias ( $\sigma(u_{0j},u_{1j})$) são 0.
...
No software `Stata` o parâmetro __covariance(independent)__ define esssa estrutura.Esse é o parâmetro _default_ do melogit
...
Salientamos que a correlação interclasse não se altera quando existem variáveis no nivel 2, pois ele é estimado por meio do modelo nulo, ou seja, por um modelo que não inclui variáveis explicativas.
