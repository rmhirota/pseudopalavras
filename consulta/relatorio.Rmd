...
Após o ajuste de um modelo de efeitos mistos logísticos com melogit pode-se encontrar diversas outras medidas e estatísticas. Os efeitos aleatórios não são fornecidos como estimativas quando o modelo é ajustado,logo, eles precisam ser previstos. Cálculo de correlações intraclasse e probabilidades de resposta positiva (=1) em cada cluster também pode ser de interesse.
...
-Exemplo com o modelo da pesquisadora (sem inclinações que variam entre os cursos)
...
re1 = estimação da parte aleatório do intercepto
2) generate $$prob\_curso = exp(pred\_efeitos\_aleat\_re1)/(1 + exp(pred\_efeitos\_aleat\_re1))$$ 

prob_curso é a probabilidade média do aluno evadir no primeiro ano em cada curso
