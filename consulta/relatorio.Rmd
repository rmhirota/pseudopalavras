...
$u_{0j}$ é o efeito aleatório do intercepto cuja distribuição segue uma N(0,$\sigma_{u_{0j}}$)
Já os coeficientes $\beta_0$,$\beta_1$,$\beta_2$,$\beta_3$,$\beta_4$ e $\beta_5$ são os efeitos fixos do modelo
...
calculados. O cálculo de correlações intraclasse (ICC) e as probabilidades de 
resposta positiva (=1) em cada cluster também pode ser de interesse.
...
$$ ICC = \frac{\sigma_{u_{0j}}}{\sigma_{u_{0j}} + (\pi^2/3)} $$
...
1. Estimação dos efeitos aleatórios
...
substituir a fórmula do modelo pelos valores encontrados para entender o
...
`rxb*` = estimação da parte constante do intercepto + parte aleatória do 
intercepto + parte da variável sexo  

3. `generate prob_curso_masc = exp(rxb_masc)/(1 + exp(rxb_masc))`

   `generate prob_curso_fem = exp(rxb_fem)/(1 + exp(rxb_fem))`

`prob_curso_masc` é a probabilidade média do aluno (do sexo masculino) evadir no
primeiro ano em cada curso.

`prob_curso_fem` é a probabilidade média da aluna (do sexo feminino) evadir no
primeiro ano em cada curso.

A discrepância entre as probabilidades de evasão por sexo em cada curso se dá 
por conta do efeito fixo do sexo, ou seja, quando a variável sexo muda (0 = 
feminino, 1 = masculino) o coeficiente referente a essa variável é acrescentado 
na equação do modelo e, consequentemente, afeta os valores finais das
probabilidades. 

Lembrando que o efeito é fixo, logo, ele não varia por curso. Se há interesse na
variação dessa influência por curso, é necessário considerar a variável como um 
fator aleatório. 
