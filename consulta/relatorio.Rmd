...
Um modelo multinível pode ser descrito da seguinte forma:

$$ logit (\pi_{ij}) = (\beta_0 +u_{0j})+\sum_{k}{ \beta_k*X_{kij}} + \epsilon_{ij} $$ 
  
para $j = 1,...,n$ clusters de cursos, com $i = 1,..., n_{j}$ alunos no cluster $j$
...
coeficientes $\beta_k$ são os efeitos fixos do modelo e $\epsilon_{ij}$ é o erro
...
No nível 2(relacionadas ao curso), inicialmente a pesquisadora criou uma 
