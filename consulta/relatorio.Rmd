...
A forma generalizada de um modelo multínivel pode ser definido da seguinte form:

$$ logit (\pi_{ij}) = \beta_{0j}+\sum_{k}{ \beta_{kj}*X_{kij}} + \epsilon_{ij}  $$ (1)
com $$ \beta_{0j} = \gamma_{00} + \sum_{a}{\gamma_{0a}}*Z_{aj} + u_{0j}$$  
 e   $$ \beta_{kj} = \gamma_{k0} + \sum_{a}{\gamma_{ka}}*Z_{aj} + u_{kj}$$

para $j = 1,...,n$ clusters de cursos, $i = 1,..., n_{j}$ alunos no cluster $j$, $k = 1,...,m$ variáveis independentes de nível 1 e $a = 1,...,h$ variáveis independentes de nível 2.
$u_{0j}$ é o efeito aleatório do j-ésimo cluster que compõe o intercepto, cuja 
distribuição segue uma N(0,$\sigma^{2}_{u_{0j}}$). $u_{kj}$ são os efeitos aleatórios da k-ésima variável no j-ésimo cluster e cuja distribuição segue uma N(0,$\sigma^{2}_{u_{kj}}$). Os erros $\epsilon_{ij}$ têm distribuição binomial com média zero, são independentes dos efeitos aleatórios mas não são independentes entre eles, ou seja, há uma dependência entre as observações agrupadas dentro de cada um dos clusters.
...
$\gamma_{00}$ e os $\gamma_{k0}$ são os coeficientes de regressão associados as variáveis explicativas do nível 2 relativos ao intercepto. Já $\gamma_{0a}$ e os $\gamma_{ka}$ são os coeficientes de regressão associados as variáveis explicativas do nível 2 relativos à inclinação
...
Um modelo multinível mais simples - consequentemente muito utilizado - é o modelo com intercepto aleatório e sem variáveis de segundo nível. Esse foi o modelo hierárquico testado inicialmente pela pesquisadora e pode ser descrito da seguinte forma:

$$ logit (\pi_{ij}) = \beta_{0j} + \sum_{k}{ \beta_{k}*X_{kij}} + \epsilon_{ij} $$  (2)
...
Nesse caso, nota-se que os interceptos variam de cluster para cluster (considera-se que possa existir diferenças entre os clusters), mas $\beta_{00}$ e os $\beta_{k0}$ são constantes para todos os clusters. 
...
__Passo 0__: Definir se o modelo terá efeito aleatório apenas no intecepto (2) ou se aprsentará efeito aleatório em outras variáveis;

__Passo 1__: Ajuste do modelo sem variáveis independentes (modelo nulo) para
calcular o coeficiente de correlação intraclasse e interpretar se há variação entre os clusters;
  
__Passo 2__: Incluir todas as variáveis independentes de nível 1 e observar a significância de cada uma. Excluir as que apresentarem p-valor > 0.05 e testar novamente o modelos

__Passo 3__: Repetir o passo anterior até que todas as variáveis no modelo apresentem significância estatística. Esse será o modelo final que deverá ser analisado.


Seguindo os passos anteriores, a pesquisadora pode construir duas linhas de modelos diferentes, um apenas com intecepto aleatório e outro com efeito aleatório no intercepto e em alguma variável de interesse, e depois compará-los através de alguma estatística. Também pode comparar um modelo sem variáveis de nível 2 e outro com variáveis de nível 2 ou variações de um mesmo modelo.

De forma geral, caso esteja em dúvida entre modelos aninhados, isto é, um é um caso particular
do outro -- apresenta um maior número de parâmetros que a primeira -- pode-se realizar um teste de razão de verossimilhança comparando-os. Como exemplo deixamos descrito o teste entre um modelo $A_1$ e um modelo reduzido $A_0$ (caso particular de $A_1$).
...
em que, nesse caso, $likelihood_0$ é a verossimilhança do modelo reduzido $A_0$ e $likelihood_1$ é a verossimilhança do modelo $A_1$. O _deviance_ tem distribuição qui-quadrado com graus de liberdade ($l$) igual a diferença do número de parâmetros em cada modelo. Se o p-valor < 0.05, então há indicíos para rejeitar $H_0$, ou seja, o modelo mais simples não parece se ajustar tão bem quanto o modelo $A_1$. 
...
do outro, usamos as medidas AIC e BIC descritas a seguir:

$$AIC = -2ln(likelihood) + 2k$$
$$BIC = -2ln(likelihood) + ln(N) k$$
Sendo $k$ o número de parâmetros estimados e $N$ o número de observações.  
Nesse caso, não há distribuição de probabilidades envolvidas, logo, quanto menor os valores do BIC/AIC, melhor o ajuste do modelo.
...
Após encontrar o modelo final é importante avaliar se o modelo é eficiente para descrever a relação entre as variáveis independentes e a variável dependente. Assim, devemos verificar se os pressupostos do modelo foram respeitadas através.
...
Do ponto de vista do nível 1 (estudantes), deve-se observar no diagnóstico do modelo
 se a função estimada é monotônica e se tem forma sigmoidal, através de um gráfico do resíduos do nível estudantes ($\epsilon_{ij}$) no eixo y e as observações no eixo x.
...
número de filhos. A idade foi centralizada por escolha dos pesquisadores do artigo, 
uma vez que a variável Idade igual a zero não traz informação relevante (mulheres
...
ajuste. Se a hipótese nula é rejeitada, existem evidências de que o modelo com efeito aleatório de cluster se ajusta melhor do que o modelo sem esse efeito
Como p-valor < 0.001, há indícios para rejeitar a hipótese nula e utilizar,
...
A tabela de estimativa acima relata os efeitos fixos do modelo e os componentes de variância estimados. Os efeitos fixos estão no formato logit tradicional. Como queremos a razão de chances em vez dos coeficientes na escala logit, devemos exponenciar as estimativas e os intervalos de confiança para encontrar um resultado interpretável. Podemos fazer 
isso no `Stata` usando a opção __OR__. 

Transformando em razão de chances, considerando mulheres em um mesmo distrito e mantidas as demais variáveis constantes, descobre-se que a chance das mulheres em zona urbana usarem anticoncepcionais é o dobro das mulheres em zona rural ($e^{0.7322765}$ = 2.07981). Além disso, ter qualquer número de filhos aumentará as chances de três a quatro vezes em comparação com a categoria base de não ter filhos ($e^{1.116001}$ = 3.052622, $e^{1.365895}$ = 3.919229 e $e{1.344031}$ = 3.834469) e o uso de anticoncepcionais diminui com o aumento de uma unidade na idade ($e^{-0.0264981}$ = 0.9738499)
...
Salientamos que as variáveis independentes foram fixadas no valor 0, ou seja, a casela de referência são mulheres que vivem no meio rural (urban = 0), com nenhum filho (1.children = 2.childre = 3.children = 0) e idade média.
...
Salientamos que as demais variáveis independentes são fixadas no valor 0.
...
2. __`generate rxb_urban  = (_b[_cons] + pred_efeitos_aleat_re1) + _b[1.urban]*1`__
   ...
   __`generate rxb_fem = (_b[_cons] + pred_efeitos_aleat_re1) + _b[1.urban]*0`__
...
2. __`generate rxb_1  = (_b[_cons] + pred_efeitos_aleat_re1) + _b[1.urban]*1` + _b[1.children]*1__
   

   __`generate rxb_2 = (_b[_cons] + pred_efeitos_aleat_re1) + _b[1.urban]*0 +_b[1,children]*0 `__

Logo,

$$ rxb_{1} = log(\frac{\pi_{j}}{1-\pi_{j}}) = (\hat\beta_0 +\hat{u_{0j}})+ \hat\beta_1*1 + \hat\beta_3*1$$
$$rxb_{rural} = log(\frac{\pi_{j}}{1-\pi_{j}}) = (\hat\beta_0 +\hat{u_{0j}})+ \hat\beta_1*0 + \hat\beta_3*0$$

`rxb_1` e `rxb_2` são as predições marginais do logaritimo da chance para
mulheres que vivem no ambiente urbano, possuem 1 filho e têm idade centrada na média
e mulheres que vivem no ambiente rural, não possuem filho e têm idade centrada na média, respectivamente
...
os cluster, ou seja, não há variação dentro ddo cluster. 

A seguir, apresentamos a fórmula matemática da métrica para um modelo com intercepto aleatório
...
Ou seja, um ICC = 0.30 indicaria que 30% da chance de evasão na UFRJ é explicada
pela diferença entre os cursos e 70% da chance de evasão é explicada pelas
