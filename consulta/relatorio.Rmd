...
continha áreas urbanas ou rurais (variável urban) ou ambas. A variável c_use é a resposta binária, com um valor de 1 indicando o uso de anticoncepcionais. Outras covariáveis incluem idade centrada na média e uma variável fatorial para o número de filhos. Considere um modelo de regressão logística padrão, alterado para ter efeitos aleatórios para cada distrito:
...
O objetivo de introduzir um coeficiente aleatório na variável binária urbana foi permitir efeitos aleatórios separados, dentro de cada distrito, para as áreas urbanas e rurais desse distrito. Logo, estamos levando em conta que dentro de cada distrito o corportamento dessa variável é diferente

No software Stata a equação é dada por:  
__melogit c_use i.urban age i.children, nofvlabel|| district: 1.urban covariance(unstructured)__

OBS: Ao especificar a covariância (não estruturada) acima, dissemos ao melogit para permitir a correlação entre efeitos aleatórios no nível distrital
...
```
A última seção nos dá as estimativas de efeito aleatório. Isso representa o desvio padrão estimado do intercepto e da inclinação na escala logit. 
