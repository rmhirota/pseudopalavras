...
A forma mais adequada de analisar os resíduos do modelo é compará-los com as probabilidades preditas, para cada nível da variável resposta. Existem observações onde a classificação do modelo não foi satisfatória, ou seja, a probabilidade predita de a resposta ser daquele nível foi baixa, e portanto o resíduo é alto. Entretanto, a frequência dessas observações é baixa, o que indica um ajuste razoável do modelo. A visão gráfica desses resíduos pode ser analisada nos gráficos B. , B. e B. , para oxítonas, paroxítonas e proparoxítonas, respectivamente. 
...
```{r}
r2 <- matrix(mf2$deviance.residuals, 3,10393) %>%
  t() %>%
  tibble::as_tibble() %>%
  purrr::set_names("paroxitona", "oxitona", "proparoxitona")
